...
async def next_message(user_message: Union[TextMessage, CodeChangeMessage]):
    ...
    message_content = get_log_message_content(user_message)

    (query_embedding, context) = retrieve_context(message_content, embeddings)

    for record in context:
        # print file_path, chunk_index, content
        print(f"file[{record.chunk_index}]: {record.file_path}")
        print(record.content)

    async def dummy_response():
        yield "Hello"

    return StreamingResponse(dummy_response(), media_type="text/plain")
