...
from backend.watchfiles import list_files_in_directory, read_ignore_patterns
...
def test_list_files_in_directory():
    ...
    with patch("os.walk", mock_os_walk), \
         patch("backend.watchfiles.read_ignore_patterns", return_value=PathSpec.from_lines(GitWildMatchPattern,  ["*.txt"])):
